...
def test_generate_text():
    ...
    response = test_client.post(
        "/generate/text/", json={"contents": "What is one plus one?"}
    )
...
async def test_generate_text_from_speech_file_by_live_api():
    ...
    **参考**
    ...
    async with client.aio.live.connect(model=model, config=config) as session:
        ...
        audio_bytes = wavefile_to_pcmbytes(
            filepath=filepath, load_samplerate=load_samplerate
        )

        await session.send_realtime_input(
            audio=types.Blob(
                data=audio_bytes, mime_type=f"audio/pcm;rate={load_samplerate}"
            )
        )
...
def assert_valid_wave_file(filepath, channels, sampwidth, framerate):
    with wave.open(filepath, "rb") as check_wf:
        ...
        print(
            f"WAV情報: チャンネル数={check_wf.getnchannels()}, サンプル幅={check_wf.getsampwidth()}, フレームレート={check_wf.getframerate()}, 総フレーム数={n_frames}"
        )
...
async def test_generate_audio_from_user_audio_with_live_api():
    ...
    async with client.aio.live.connect(model=model, config=config) as session:
        ...
        audio_bytes = wavefile_to_pcmbytes(
            filepath=input_filepath, load_samplerate=framerate
        )
